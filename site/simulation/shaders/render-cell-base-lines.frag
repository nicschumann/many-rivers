...
void main() {
    ...
    vec3 base_unchanged_color = vec3(0.2, 0.2, 0.3) * 1.2;
    vec3 base_changed_color = vec3(0.80, 0.73, 0.05);
    ...
    if (v_W > 0.0) {
      gl_FragColor = vec4(vec3(0.95), u_alpha);
    } else {
      gl_FragColor = vec4(vec3(0.35), u_alpha);
    }
  ...
  //   gl_FragColor = vec4(color, u_alpha);
  //  } else {
  //   gl_FragColor = vec4(base_unchanged_color, u_alpha);
  //  }   
}